...
// ye hmara middleware hai (global)
app.use(express.json());
// agar kisi request ke andr JSON body aa rhi hai..to ye usko req me se nikalke combine krke req k andr hi body ke naam se add krdega

app.get("/", function (req, res) {
  res.sendFile(__dirname + "/assign9.html");
});

app.post("/todo", function (req, res) {
  saveTodoInFile(req.body, function (err) { // file mai task save krna hai
    if (err) {
...
app.post("/delete-todo",function (req,res){
  removeTodoFromFile(req.body,function (err){ /// Jis file se remove krna hai
    if (err){
      res.status(500).send("error");
    }
    else{
...
app.get("/assign9.js", function (req, res) {
  res.sendFile(__dirname + "/assign9.js");
});
...
function readAllTodos(callback) {
  fs.readFile("./assign9.json", "utf-8", function (err, data) {
    if (err) {
      callback(err);
      return;
    }
    // length agr 0 hai to blank array bnado
    if (data.length === 0) {
      data = "[]";
    }
...
function saveTodoInFile(todo, callback) {
  readAllTodos(function (err, data) {
    ...
    fs.writeFile("./assign9.json", JSON.stringify(data), function (err) {
      if (err) {
        callback(err);
        return;
      }
